...
def patrol_calculator():
    time_input = input("⏱ Введіть тривалість польоту у форматі ГГ:ХХ (наприклад 8:15): ")

    try:
        hours, minutes = map(int, time_input.split(":"))
    except ValueError:
        print("❌ Невірний формат! Використовуйте вигляд ГГ:ХХ (наприклад 8:15)")
        return
    ...
    # Розрахунки
    int_hours_value = hours - 1
    int_minutes_value = minutes
    ...
    print("------------------")
    print(
        f"Загальна формула : За першу годину польоту -> 1400 мл. "
        f"+ решта часу : {int_hours_value} годин -> {calculate_last_hours} мл."
    )
